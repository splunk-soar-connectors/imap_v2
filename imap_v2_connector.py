...
from imap_v2_consts import (
    IMAP_V2_CONNECTIVITY_ENDPOINT,
    IMAP_V2_ERR_CONNECTIVITY_TEST,
    IMAP_V2_SUCC_CONNECTIVITY_TEST,
)


class Imap_v2Connector(BaseConnector):
    """
    Imap_v2 connector class that serves as a starting point for new connectors.
    """
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = IMAP_V2_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(IMAP_V2_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(IMAP_V2_SUCC_CONNECTIVITY_TEST)
